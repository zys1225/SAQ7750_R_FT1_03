...
BOOL BoardCheck::test_v(ACM &acm_measure, DWORD tnum, const char* component, double lolim, double hilim, const char* unit, double *result1)	// dual ch fovi shorted, one force v, one measure v
{
	double result[SITE_NUM];
	acm_measure.Set(FI, 0, ACM_N2P18V, ACM_5UA, ACM_RELAY_ON);
	delay_ms(5);
...
BOOL BoardCheck::test_i(const char* lpszGroupPinName, const char* lpszPinName, DWORD tnum, const char* component, double fv,double lolim, double hilim, const char* unit, double *result1)
{
	...
	dcm.Connect(lpszGroupPinName);
	dcm.SetPPMU(lpszGroupPinName, DCM_PPMU_FVMI, fv, DCM_PPMUIRANGE_200UA);
	delay_ms(50);
